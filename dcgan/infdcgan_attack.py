...
for epoch in range(opt.niter):
    for i, batch in enumerate(dataloader):
        ...
        ###########################
        netG.zero_grad()
        del_x = torch.max(torch.sum(torch.abs(fake),1))
        output = output_fake + c_g * del_x.pow(2)
    ...
    if epoch % 10 == 0:
        vutils.save_image(real_cpu[0:32],
                '%s/infinity_real_samples.png' % opt.outf,
                normalize=True)
        fake = netG(Variable(inputv.grad.data)) + inputv
        vutils.save_image(fake.data[0:32],
                '%s/images/infinity_fake_samples_epoch_%03d.png' % (opt.outf, epoch),
                normalize=True)

    # do checkpointing
    torch.save(netG.state_dict(), '%s/infinity_netG_epoch_%d.pth' % (opt.outf, epoch))
    torch.save(netD.state_dict(), '%s/infinity_netD_epoch_%d.pth' % (opt.outf, epoch))
